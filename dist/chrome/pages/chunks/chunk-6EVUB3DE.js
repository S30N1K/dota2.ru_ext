...
// src/storage.ts
var log = new extLogger("storage.ts");
var extensionUrl = ref("");
var currentUser = reactive({ ...DEFAULT_CURRENTUSER });
var chatSettings = reactive({ ...DEFAULT_CHAT });
var settings = reactive({ ...DEFAULT_SETTINGS });

// src/utils/api.ts
var log2 = new extLogger("utils/api.ts");
async function parseJson(url, data, options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      log2.error(`HTTP error! status: ${response.status}`);
    }
...
async function parseText(url, options = {}) {
  const controller = new AbortController();
  const timeoutId = options.timeout ? setTimeout(() => controller.abort(), options.timeout) : null;
  try {
    const response = await fetch(url, {
...
vue/dist/vue.runtime.esm-bundler.js:
  ...
  **)
*/
//# sourceMappingURL=chunk-6EVUB3DE.js.map
