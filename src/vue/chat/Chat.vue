<template>
  ...
  >
    ...
    <template #default>
      <ChatUserProfile ref="userProfile" @openProfile="openProfile" @mention="mention"/>
      <div class="chatBody" ref="chatBody">
        ...
        <div v-show="!isValidConnected" class="chatContainer">
          <ChatMessageList @openUserProfile="openUserProfile" @insertUser="chatFooter?.insertUser" ref="messageList"/>

          <ChatNotifications @openUserProfile="openUserProfile" :isOpen="envelopeIsOpen"/>

          <UsersList
              @insertUser="chatFooter?.insertUser"
...
const userProfile = ref<InstanceType<typeof ChatUserProfile> | null>(null)
const chatBody = ref<InstanceType<typeof HTMLElement> | null>(null)
...
const openProfile = (user: UserChat) => {
  window.open('https://dota2.ru/forum/members/.' + user.id + '/', '_blank', 'noopener,noreferrer');
}

const mention = (user: UserChat) => {
  chatFooter.value?.insertUser(user)
}

const openUserProfile = (e: MouseEvent, user: UserChat) => {
  userProfile?.value?.open(e.clientX, e.clientY, user, chatBody.value as HTMLElement)
}
